...
export class Game {
  ...
  async startRun(seed = null, instant = false) {
    ...
    track('run_start', { run_id: this._runId, run_index: this._runIndex, dog: this.breed, seed: this.seed, mult: this.metaMult });
  ...
  _weaveTap(dir) {
    ...
    this.score += 15 * Math.min(SCORE_COMBO_CAP, Math.max(1, this.combo)) * (this.metaMult || 1);
  }
        ...
        if (dz < (e.halfD + 0.3) && ddx < (e.halfW + 0.28)) {
          if (e.kind === 'cone') {
            ...
            if (!e._nearMissed && !over && d.stumbleInvulnT <= 0 && ddx >= e.halfW + pad && ddx < e.halfW + pad + 0.55) {
              ...
              this.score += 30 * Math.min(SCORE_COMBO_CAP, Math.max(1, this.combo)) * (this.metaMult || 1);
  ...
  _obstacleClean(e, grade, name, baseScore) {
    ...
    const mult = Math.min(SCORE_COMBO_CAP, Math.max(1, this.combo)) * (this.powerups.multi > 0 ? 2 : 1) * (this.tableBoostT > 0 ? 2 : 1) * (this.metaMult || 1);
    let pts = base * mult;
    if (grade === 'perfect') {
  ...
  _death(e, disqualified = false) {
    if (this.ftue && this.state === 'running') { this._ftueFail(); return; } // в обучении смерти нет
