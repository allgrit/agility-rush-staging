...
export class Dog {
  ...
  update(dt, pose) {
    ...
    if (mode === 'run' || mode === 'weave' || mode === 'balance') {
      ...
      if (mode === 'balance') {
        // ЗНАК: rotation.z>0 валит верх в −X (влево экрана), а сдвиг тела идёт в +X —
        // крен обязан совпадать со сдвигом, иначе игрок читает падение наоборот.
        bodyRoll = -(p.balance || 0) * 1.05;
        bodyY = 0.40;
