...
export class Meta {
  ...
  _load() {
    ...
    return {
      ...
      qualityTier: null, // осевшая ступень адаптивного качества (0..3); локально, НЕ в облако
    };
  }
  ...
  _migrate(d) {
    // Обучение по умолчанию у ВСЕХ, даже игравших: «играл» не значит «знает правила».
    // Пропуск — только явной кнопкой в туторе (ftue_skip).
    if (d.ftueDone === undefined) d.ftueDone = false;
